...
*/
    ...
    int nroDePares = 0;         //vai representar o das diversas colunas
    int index = 0; 
    
    int tamArrayFinal;          //usar delete coluna e fazer coluna de novo
    
    vector<mintermo_> arrayFinal; 

    //sera iterado para achar pares:
    //coluna0
    //nroDadosColuna                    <--- info importante para as funcoes

    //eh o resultado dessa iteracao:
    //coluna1                           <--- coluna com todos os pares da coluna0
    //nroDePares                        <--- tambem eh o tamanho da coluna 1 
    //                                  <--- se eh zero, continua = false
    ...
    while (continua) {
        ...
        if (nroDadosColuna == 0) {
            continua = false;
            //pois a ultima coluna nao havia entrado, entao preciso colocar essa coluna 0
            for (int i = 0; i < nroDadosColuna; i++) {
                if (!get<2>(coluna0[i])){
                    arrayFinal.push_back(coluna0[i]); 
                    tamArrayFinal++; 
                }
            }
        }
            
        //se essa coluna nao tem nenhum par, ja paro nela
        else {
            nroDePares = encontraPares(coluna0, nroDadosColuna,qntdVariaveis);

            mintermo_ coluna1[nroDePares];              //criando coluna pros novos valores 

            for(int i = 0; i < nroDePares; i++) {       //limpando todos os dados dessa coluna vazia 
                get<1>(coluna1[i]) = ""; 
            }

            index = 0; 
            constroiArrayPares(coluna0, nroDadosColuna, qntdVariaveis, coluna1, nroDePares, index);     //construindo array
            ordenaColuna(coluna1, nroDePares,qntdVariaveis-1);      //posso ir mudando o numero de pares pq no final vou conseguir o numero de todos iterando

            cout << "\n\ncoluna " << contagemColunas << ": \n";
            for (int i = 0; i<nroDePares; i++){
                cout << get<3>(coluna1[i]) << ": " << get<1>(coluna1[i]) << "\n";
            } 
            contagemColunas++;  //digo que vou pra proxima coluna

            
            //colocando no array final a coluna anterior (pois já pode ser marcada)
            for (int i = 0; i < nroDadosColuna; i++) {
                if (!get<2>(coluna0[i])){
                    arrayFinal.push_back(coluna0[i]); 
                    tamArrayFinal++; 
            ...
            cout << "ba O";
            nroDadosColuna = nroDePares; 


            //crio nova coluna 0 com o que era o array 1, e com seu tamanho
            //delete[] coluna0; 
            cout << "oizinho";
            mintermo_ coluna0[nroDePares]; 
            copy(coluna1, coluna1 + nroDePares, coluna0); 

            cout << "testando coluna\n";
            for (int i = 0; i < nroDePares; i++) 
                cout << get<3>(coluna0[i])<< " ";


            //a coluna 1 eh criado no laco
            //delete[] coluna1; 
            cout << "nroDados" << nroDadosColuna; 
...
*/
    ...
    mintermo_ arrayFinalA[tamArrayFinal];

    for (int i = 0; i < tamArrayFinal; i++) {
        arrayFinalA[i] = arrayFinal[i];
    } 
    desenhaTabela (mintermos, arrayMintermos, nroMintermos, arrayFinalA, tamArrayFinal);
    return 0; 
}
...
int constroiArrayPares (mintermo_* coluna, int dadosColuna, int nroVariaveis, mintermo_* novaColuna, int dadosNovaColuna, int idex) {
    int traco = -1;
    int nroDePares = 0;
    int index = 0; 
